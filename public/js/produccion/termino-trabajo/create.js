...
function main() {
    ...
    $body.on('click','[data-save_end_work]',save_end_work);
}
...
function search(e) {
    if( e.which == 13 || e.which == 1 ) {
        ...
        if( $worker_code.length == 0 ) {
            showmessage('Ingrese código del trabajador.', 0);
            return;
        }
...
function search_order(e) {
    if( e.which == 13 || e.which == 1 ) {
        ...
        var $url = $('#url_search_order').val()+'/'+$worker_code+'/'+$search_order;
        $.ajax({
            url:$url,
            type:'get'
        }).done(function (data) {
            if( data.success=='true'){
                ...
                var $total_pairs = $('#total_pairs');
                $pairs.val(data.data);
                $disponible.val(data.data - data.cant_asignada);
                $total_pairs.val(data.order);
            }else
                showmessage(data.message,0)
        });
    }
...
function save_end_work() {
    ...
    var $url = $('#url_termino_trabajo_store').val();
    ...
    if( $data.date.length == 0 ) {
        showmessage('Seleccione la fecha de fin.',0);
        return;
    }
    ...
    if( $data.pairs_user < 1 ) {
        showmessage('La cantidad de pares a terminar.',0);
        return;
    }

    if( parseInt($data.pairs_user) > parseInt($pairs) ) {
        showmessage('La cantidad de pares a terminar, no puede ser mayor a la cantidad de pares del inicio de trabajo.',0);
        return;
    }
    ...
    $.ajax({
        ...
        if( data.success=='true'){
            ...
            setTimeout(function () {
                var $back_end = $('#back_end');
                location.href = $back_end.attr('href');
            },500);
        }else
            showmessage(data.message,0)
    });
